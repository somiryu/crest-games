...
public class FrustrationThermometerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < frustrationLevels.Count; i++)
        {
            ...
            if (UserDataManager.CurrUser.gender == UserSex.Hombre) frustrationLevels[idx].FLabel.gameObject.SetActive(false);
    ...
    IEnumerator Intro()
    {
        ...
        for (int i = 0; i < frustrationLevels.Count; i++)
        {
            ...
            continueBtn.gameObject.SetActive(false);
            var currAudio = UserDataManager.CurrUser.gender == UserSex.Mujer ? currDescription.FbuttonSound : currDescription.MbuttonSound;
    ...
    void GetFrustationLevel(FrustrationLevels level)
    {
        ...
        if(UserDataManager.CurrUser.gender == UserSex.Hombre) audioSource.clip = level.MbuttonSound;
