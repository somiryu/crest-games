...
public class FrustrationThermometerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < frustrationLevels.Count; i++)
        {
            ...
            if (UserDataManager.CurrUser.gender == UserSex.Masculino) frustrationLevels[idx].FLabel.gameObject.SetActive(false);
    ...
    IEnumerator Intro()
    {
        ...
        for (int i = 0; i < frustrationLevels.Count; i++)
        {
            ...
            continueBtn.gameObject.SetActive(false);
            var currAudio = UserDataManager.CurrUser.gender == UserSex.Femenino ? currDescription.FbuttonSound : currDescription.MbuttonSound;
    ...
    void GetFrustationLevel(FrustrationLevels level)
    {
        ...
        if(UserDataManager.CurrUser.gender == UserSex.Masculino) audioSource.clip = level.MbuttonSound;
